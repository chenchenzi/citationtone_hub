...
contour per tone, use \[compute_mean_contour()\], which puts time on a
`[0, 1]` axis before averaging: it rescales within each token, unless
the time column is already proportional, which it detects and leaves
alone (`time_normalised` overrides either way). The package function
