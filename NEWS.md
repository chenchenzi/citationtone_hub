...
* **Landmark time normalisation** (new). `normalise_time_landmarks()` rescales
  ...
  a sequential, word-level axis (`<tier>_tseq`). Surfaced through a new "Time
  Normalisation" section on the Normalise tab. The model tabs default the Time
  variable to `<tier>_tseq` when present and steer multisyllabic analyses
  toward GAMM.
