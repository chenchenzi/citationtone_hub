    ...
    tagList(
      tags$div(
        ...
        tags$ul(style = "margin-bottom: 8px; padding-left: 18px;",
          tags$li(tags$strong("Token ID:"), " Column uniquely identifying each syllable/token. One polynomial fit per token."),
          ...
          tags$li(tags$strong("Time:"), " The time variable within each token. Normalised to [-1, 1] per token before fitting."),
  ...
  output$ui_model <- renderUI({
    ...
    tagList(
      wellPanel(
        ...
        selectInput("model_f0_var", "Select f0 variable (normalised recommended):",
